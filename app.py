...
def upload_img():
    ...
    newFile = iDetails(imgfile=file.read())
    return jsonify(message="adhar card front saved ")


@app.route('/upload_adhaar_front',methods=['POST'])
#@jwt_required
def upload_adhaar_front():
    file = request.files['Adhar_card_front']
    newFile = iDetails(Adhar_card_front=file.read())
    db.session.add(newFile)
    db.session.commit()
    return jsonify(message="adhar card front saved ")


@app.route('/upload_adhaar_back',methods=['POST'])
#@jwt_required
def upload_adhaar_back():
    file = request.files['Adhar_card_back']
    newFile = iDetails(Adhar_card_back=file.read())
    db.session.add(newFile)
    db.session.commit()
    
    return jsonify(message="adhar card back saved ")
...
def upload_pan():
    ...
    newFile = iDetails(pan=file.read())
    db.session.add(newFile)
    db.session.commit()
    
    return jsonify(message="pan saved ")


@app.route('/upload_bank_statement',methods=['POST'])
#@jwt_required
def upload_bank_statement():
    file = request.files['bank_Statement']
    newFile = iDetails(bank_Statement=file.read())
    db.session.add(newFile)
    db.session.commit()
    
    return jsonify(message="bank statement saved ")
...
class Investor(db.Model):
    ...
    phone = Column(Float, nullable=False, unique=True)
...
class iDetails(db.Model):
    ...
    inv_type = Column(String, nullable= True)
    equity = Column(Float,nullable=True)
    no_of_members = Column(Integer,nullable=True)
    cash_at_hand = Column(Float,nullable=True)
    interested_investing = Column(String,nullable=True)
    role = Column(String,nullable=True)
    imgfile = Column(LargeBinary,nullable=True)
    invested_companies = Column(String,nullable=True)
    list_of_companies = Column(String,nullable=True)
    # kyc upload files
    Adhar_card_front = Column(LargeBinary,nullable=True)
    Adhar_card_back = Column(LargeBinary,nullable=True)
    pan = Column(LargeBinary,nullable=True)
    bank_Statement = Column(LargeBinary,nullable=True)
